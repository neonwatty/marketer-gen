...
import { Archive, Copy, Edit, Eye, FileText, MoreHorizontal, TrendingUp, Users, Loader2 } from 'lucide-react'
...
export function EnhancedCampaignCard({
  ...
  const isAnyActionLoading = isUpdating || isDuplicating || isArchiving
  ...
  return (
    <Card className={`group relative transition-all hover:shadow-lg ${
      isAnyActionLoading ? 'pointer-events-none opacity-70' : ''
    }`}>
      {/* Loading overlay for card-level operations */}
      {(isUpdating || isArchiving) && (
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
                ...
                <DropdownMenuContent align="end" className="w-[160px]">
                  <DropdownMenuItem 
                    onClick={() => handleAction(() => onView?.(campaign.id))}
                    disabled={isAnyActionLoading}
                  >
                  ...
                  <DropdownMenuItem 
                    onClick={() => handleAction(() => onDuplicate?.(campaign.id))}
                    disabled={isAnyActionLoading}
                  >
                    {isDuplicating ? (
