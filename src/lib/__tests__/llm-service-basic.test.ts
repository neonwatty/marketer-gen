...
describe('LLM Service - Basic Functionality', () => {
  ...
  describe('Request Validation', () => {
    ...
    it('should reject invalid maxTokens values when defined', async () => {
    ...
    it('should reject invalid temperature values when defined', async () => {
      const invalidRequests = [
  ...
  describe('Processing Time Simulation', () => {
    ...
    it('should vary processing times', async () => {
      const requests = Array(3).fill({ prompt: 'Test variation' })
      const results = await Promise.all(requests.map(req => service.generateContent(req)))
