...
class AbTestingSystemComprehensiveTest < ActiveSupport::TestCase
  ...
  test "should generate multiple test variants automatically" do
    ...
    generated_variants[:variants].each_with_index do |variant, index|
      ...
      if index == 0
        assert_equal "control", variant[:type]  # First should be control
        assert_equal @journey_control.id, variant[:journey_id]
      else
        assert_equal "generated", variant[:type]  # Rest should be generated
        assert_not_equal @journey_control.id, variant[:journey_id]
      end
    end
  end
